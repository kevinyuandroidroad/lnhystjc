 ...
 <form class="form-horizontal" role="form" id="param_form" action="">
  ...
  <div class="panel panel-default">
   <div class="panel-heading">
    <h3 class="panel-title">点对点消息传输</h3>
   </div>
   <div class="panel-body">
    ...
    <div class="row">
     <div class="form-group col-sm-9">
      <label for="lastname" class="col-sm-3 control-label">输入参数超时时间</label>
      <div class="col-sm-9">
       <div class="col-sm-6">
        <input class="form-control" name='pro_${record.proMap["pro_input_value"].proId }' type="text" placeholder="数值"
         value='${record.proMap["pro_input_value"].proValue }' />
       </div>
       <div class="col-sm-6">
        <select class="form-control" name='pro_${record.proMap["pro_input"].proId }'>
         <c:forEach var="pro" items="${time_u }">
          <c:if test='${record.proMap["pro_input"].proValue eq pro.dicProCode}'>
           <option value="${pro.dicProCode }" selected="selected">${pro.dicProTitle }</option>
          </c:if>
          <c:if test='${record.proMap["pro_input"].proValue ne pro.dicProCode }'>
    ...
    <div class="row">
     <div class="form-group col-sm-9">
      <label for="lastname" class="col-sm-3 control-label">输出参数超时时间</label>
      <div class="col-sm-9">
       <div class="col-sm-6">
        <input class="form-control" name='pro_${record.proMap["pro_out_value"].proId }'
         value='${record.proMap["pro_out_value"].proValue }' type="text" placeholder="数值" />
       </div>
       <div class="col-sm-6">
        <select class="form-control" name='pro_${record.proMap["pro_out"].proId }'>
         <c:forEach var="pro" items="${time_u }">
          <c:if test='${record.proMap["pro_out"].proValue eq pro.dicProCode}'>
           <option value="${pro.dicProCode }" selected="selected">${pro.dicProTitle }</option>
          </c:if>
          <c:if test='${record.proMap["pro_out"].proValue ne pro.dicProCode }'>
           <option value="${pro.dicProCode }">${pro.dicProTitle }</option>
          </c:if>
         </c:forEach>
        </select>
